...
class JsonServiceTest {
    ...
    void deserializeBrand() {
        // Create Test Brand for the first element in the list
        ...
        //Check that the brand that is created, is the same as the output from json
        int indexNumber = 0;
        assertEquals(known.getName(), service.deserializeBrand().get(indexNumber).getName());
        assertEquals(known.getDescription(), service.deserializeBrand().get(indexNumber).getDescription());
        assertEquals(known.getFounded(), service.deserializeBrand().get(indexNumber).getFounded());
        assertEquals(known.getHeadquarters(), service.deserializeBrand().get(indexNumber).getHeadquarters());

        for (int i = 0; i < known.getProducts().size(); i++) {
            assertEquals(known.getProducts().get(i), service.deserializeBrand().get(indexNumber).getProducts().get(i));
        }

        // Create Test Brand for the last element in the list
        known.setName("Intel");
